...
public record AccountSummary(Long accountId, String name, AccountType type, BigDecimal balance) {
  public static AccountSummary from(Account account) {
    return new AccountSummary(account.getId(), account.getName(), account.getType(), account.getBalance());
  }
}
